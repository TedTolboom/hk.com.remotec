...
class ZXT600 extends ZwaveDevice {
  ...
  async onNodeInit() {
    ...
    this.setStoreValue('thermostatSetpointValue.fixed', 25).catch(this.error);
    ...
    this.registerCapability('AC_mode', 'THERMOSTAT_MODE', {
      ...
      setParserV3: value => {
        ...
        this.setCapabilityValue('AC_onoff', value !== 'Off').catch(this.error);

        // Update thermostat mode
        this.setStoreValue('thermostatSetpointType', setPointType).catch(this.error);
      ...
      reportParserV3: report => {
        ...
        if (report.hasOwnProperty('Level') && report.Level.hasOwnProperty('Mode')) {
          ...
          // Update AC_onoff capability
          this.setCapabilityValue('AC_onoff', report.Level.Mode !== 'Off').catch(this.error);
